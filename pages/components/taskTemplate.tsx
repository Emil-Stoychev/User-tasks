...
export const TaskTemplate = ({ task, setTasks, deleteTask, changeStatus}) => {
      ...
      const onEditSubmit = async (e: any) => {
        ...
        if (data.title != "" && data.description != "") {
          const response = await fetch("/api/edit", {
            ...
            body: JSON.stringify({data: data, taskId: task._id}),
          });
    ...
    return (
        ...
        <li key={task?._id}>
            ...
            {isEdit
                ? 
                <>
                    <input type="text" value={data.title} onChange={onChangeHandler} name="title" placeholder="Title" />
                    <input type="text" value={data.description} onChange={onChangeHandler} name="description" placeholder="Description" />
            ...
            {!isEdit &&
                <>
                    <h3 onClick={() => changeStatus(task?._id, task?.completed)}>
                    Status:{" "}
                    {task?.completed 
                    ...
                    <h3 onClick={() => editTaskHandler(task._id)}>
