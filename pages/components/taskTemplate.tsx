...
export const TaskTemplate = (props: { task: Task, setTasks: Function, deleteTask: Function, changeStatus: Function}) => {
    ...
    const editTaskHandler = async () => {
      ...
      const onEditSubmit = async (e: any) => {
        ...
        if (validateData?.error?.message == undefined) {
          ...
          props.setTasks((state: any) => state.map((x: any) => {
    ...
    return (
        ...
        <li key={props.task?._id}>
            ...
            {!isEdit &&
                <>
                    <h3 onClick={() => props.changeStatus(props.task?._id, props.task?.completed)}>
                    Status:{" "}
